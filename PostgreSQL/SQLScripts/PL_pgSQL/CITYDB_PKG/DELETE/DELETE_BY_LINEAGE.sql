...
*
* FUNCTIONS:
*   delete_bridges(lineage_value TEXT, schema_name TEXT DEFAULT 'citydb') RETURNS SETOF VOID
*   delete_buildings(lineage_value TEXT, schema_name TEXT DEFAULT 'citydb') RETURNS SETOF VOID
*   delete_city_furnitures(lineage_value TEXT, schema_name TEXT DEFAULT 'citydb') RETURNS SETOF VOID
*   delete_cityobjects(lineage_value TEXT, delete_members INTEGER DEFAULT 0, schema_name TEXT DEFAULT 'citydb') RETURNS SETOF VOID
*   delete_cityobjectgroups(lineage_value TEXT, delete_members INTEGER DEFAULT 0, schema_name TEXT DEFAULT 'citydb') RETURNS SETOF VOID
*   delete_generic_cityobjects(lineage_value TEXT, schema_name TEXT DEFAULT 'citydb') RETURNS SETOF VOID
*   delete_land_uses(lineage_value TEXT, schema_name TEXT DEFAULT 'citydb') RETURNS SETOF VOID
*   delete_plant_covers(lineage_value TEXT, schema_name TEXT DEFAULT 'citydb') RETURNS SETOF VOID
*   delete_relief_features(lineage_value TEXT, schema_name TEXT DEFAULT 'citydb') RETURNS SETOF VOID
*   delete_soltary_veg_objs(lineage_value TEXT, schema_name TEXT DEFAULT 'citydb') RETURNS SETOF VOID
*   delete_transport_complexes(lineage_value TEXT, schema_name TEXT DEFAULT 'citydb') RETURNS SETOF VOID
*   delete_tunnels(lineage_value TEXT, schema_name TEXT DEFAULT 'citydb') RETURNS SETOF VOID
*   delete_waterbodies(lineage_value TEXT, schema_name TEXT DEFAULT 'citydb') RETURNS SETOF VOID
******************************************************************/
...
BEGIN
  FOR building_id IN EXECUTE format('SELECT id FROM %I.cityobject WHERE objectclass_id = 26 AND lineage = %L', schema_name, lineage_value) LOOP
    BEGIN
      PERFORM citydb_pkg.delete_building(building_id, schema_name);
...
BEGIN
  FOR bridge_id IN EXECUTE format('SELECT id FROM %I.cityobject WHERE objectclass_id = 64 AND lineage = %L', schema_name, lineage_value) LOOP
    BEGIN
      PERFORM citydb_pkg.delete_bridge(bridge_id, schema_name);
...
BEGIN
  FOR tunnel_id IN EXECUTE format('SELECT id FROM %I.cityobject WHERE objectclass_id = 85 AND lineage = %L', schema_name, lineage_value) LOOP
    BEGIN
      PERFORM citydb_pkg.delete_tunnel(tunnel_id, schema_name);
...
BEGIN
  FOR city_furniture_id IN EXECUTE format('SELECT id FROM %I.cityobject WHERE objectclass_id = 21 AND lineage = %L', schema_name, lineage_value) LOOP
    BEGIN
      PERFORM citydb_pkg.delete_city_furniture(city_furniture_id, schema_name);
...
BEGIN
  FOR generic_cityobject_id IN EXECUTE format('SELECT id FROM %I.cityobject WHERE objectclass_id = 5 AND lineage = %L', schema_name, lineage_value) LOOP
    BEGIN
      PERFORM citydb_pkg.delete_generic_cityobject(generic_cityobject_id, schema_name);
...
BEGIN
  FOR land_use_id IN EXECUTE format('SELECT id FROM %I.cityobject WHERE objectclass_id = 4 AND lineage = %L', schema_name, lineage_value) LOOP
    BEGIN
      PERFORM citydb_pkg.delete_land_use(land_use_id, schema_name);
...
BEGIN
  FOR plant_cover_id IN EXECUTE format('SELECT id FROM %I.cityobject WHERE objectclass_id = 8 AND lineage = %L', schema_name, lineage_value) LOOP
    BEGIN
      PERFORM citydb_pkg.delete_plant_cover(plant_cover_id, schema_name);
...
BEGIN
  FOR soltary_veg_obj_id IN EXECUTE format('SELECT id FROM %I.cityobject WHERE objectclass_id = 8 AND lineage = %L', schema_name, lineage_value) LOOP
    BEGIN
      PERFORM citydb_pkg.delete_soltary_veg_obj(soltary_veg_obj_id, schema_name);
...
BEGIN
  FOR transport_complex_id IN EXECUTE format('SELECT id FROM %I.cityobject WHERE objectclass_id = 42 AND lineage = %L', schema_name, lineage_value) LOOP
    BEGIN
      PERFORM citydb_pkg.delete_transport_complex(transport_complex_id, schema_name);
...
BEGIN
  FOR waterbody_id IN EXECUTE format('SELECT id FROM %I.cityobject WHERE objectclass_id = 9 AND lineage = %L', schema_name, lineage_value) LOOP
    BEGIN
      PERFORM citydb_pkg.delete_waterbody(waterbody_id, schema_name);
...
BEGIN
  FOR cityobjectgroup_id IN EXECUTE format('SELECT id FROM %I.cityobject WHERE objectclass_id = 23 AND lineage = %L', schema_name, lineage_value) LOOP
    BEGIN
      PERFORM citydb_pkg.delete_cityobjectgroup(cityobjectgroup_id, delete_members, schema_name);
...
BEGIN
  FOR relief_feature_id IN EXECUTE format('SELECT id FROM %I.cityobject WHERE objectclass_id = 14 AND lineage = %L', schema_name, lineage_value) LOOP
    BEGIN
      PERFORM citydb_pkg.delete_relief_feature(relief_feature_id, schema_name);
